...
Static site, 60 pages, generated — do not edit HTML files directly.
...
## SEO — keep maximized
- Org node: MedicalClinic+MedicalBusiness @id https://regenorthopb.com/#organization.
...
- The homepage canonical is the ROOT (`canonical=""` -> https://regenorthopb.com/), not
...
- `BASE` is the APEX, `https://regenorthopb.com` — no www. The WordPress site this
  replaced canonicalised to the apex and its sitemap emitted the apex, so every backlink,
  citation and GBP entry already points there. `BASE` and the Vercel project's primary
  domain must always agree, or all 60 canonicals point at a URL that 308s.
- vercel.json 301-maps every old WordPress URL — keep ≥12 months post-launch. It is a
  standalone file (NOT generated by build.py) — edit directly. It also carries an
  extensionless 308 for every page (`/contact` -> `/contact.html`), because the site is
  served without `cleanUrls` and bare paths would otherwise 404. Do NOT turn on
  `cleanUrls`: it would 308 every canonical the sitemap advertises. Do NOT collapse the
  extensionless entries into a `/services/:slug` wildcard either — that matches
  `x.html` and rewrites it to `x.html.html`. ADD A NEW PAGE = add its extensionless
  redirect here in the same change.
...
- `SHARE_BASE` (top of build.py) is the host for og:image/twitter:image ONLY; canonicals,
  schema @ids and the sitemap stay on `BASE`. It is now `= BASE` (the DNS flip is done).
  It exists as a separate constant because scrapers *fetch* the og:image URL — when it
  404s, iMessage/Slack silently fall back to scraping some arbitrary image off the page
  (this is why pre-launch shared links previewed with a single provider headshot). If the
  site is ever moved to a host the domain does not yet point at, park SHARE_BASE on the
  reachable deploy host again; the build prints a reminder while the two differ.
